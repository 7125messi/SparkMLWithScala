...
object TreeModel {
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.sparkContext.setCheckpointDir("file:///Users/zhaoyadong/opt/git/sparklearning/src/main/resources/TreeModel") //设置文件读取、存储的目录，HDFS最佳
    import spark.implicits._

    //1 训练样本准备
    val data = spark.read.format("libsvm").load("file:///Users/zhaoyadong/opt/git/sparklearning/src/main/resources/sample_libsvm_data.txt")
    data.show
    ...
    //6 构建Pipeline
    val pipeline1 = new Pipeline().setStages(Array(labelIndexer, featureIndexer, dt, labelConverter))

    val pipeline2 = new Pipeline().setStages(Array(labelIndexer, featureIndexer, rf, labelConverter))

    val pipeline3 = new Pipeline().setStages(Array(labelIndexer, featureIndexer, gbt, labelConverter))
    ...
    //11 模型保存与加载
    model1.write.overwrite().save("file:///Users/zhaoyadong/opt/git/sparklearning/src/main/resources/TreeModel")
    val load_treeModel = PipelineModel.load("file:///Users/zhaoyadong/opt/git/sparklearning/src/main/resources/TreeModel")
    println(load_treeModel)
  }
